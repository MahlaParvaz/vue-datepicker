...
import { isSameDate } from '../../utils/datepicker/dateComparison.js';
import { CALENDAR_CONFIG } from '../../constants/datepicker.js';
import { useDatePickerLocale } from './useDatePickerLocale.js';

export function useCalendarGrid(options) {
  const { year, month, selection, constraints, initialLocale = 'jalaali' } = options;

  const today = jalaaliToday();

  const localeState = useDatePickerLocale(initialLocale);

  if (options.locale) {
    watch(options.locale, (newVal) => localeState.setLocale(newVal));
  }
  ...
  function createDayObject(day, monthOffset = 0) {
    ...
    const date = { jy: targetYear, jm: targetMonth, jd: day };
    const key = monthOffset === 0 ? day : `${monthOffset > 0 ? 'next' : 'prev'}-${day}`;
    const label = localeState.formatNumber(day);
  ...
  return {
    ...
    numberSystem: localeState.numberSystem,
    calendarType: localeState.calendarType,
    setLocale: localeState.setLocale,
    getText: localeState.getText,
    getMonthName: localeState.getMonthName,
    getWeekdayName: localeState.getWeekdayName,
    getWeekdayFullName: localeState.getWeekdayFullName,
    availableLocales: localeState.availableLocales,
  };
}
